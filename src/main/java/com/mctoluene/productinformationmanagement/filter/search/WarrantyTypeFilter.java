...
@AllArgsConstructor
@NoArgsConstructor
public class WarrantyTypeFilter extends BaseFilter {
    ...
    public <T, D> void filter(QueryBuilder<T, D> queryBuilder) {
        ...
        if (!StringUtils.isEmpty(warrantyTypeName)) {
            queryBuilder.in("warrantyTypeName", Arrays.asList(warrantyTypeName.split(",")));
        }
    ...
    private <T, D> void filterByString(QueryBuilder<T, D> queryBuilder) {
        if (filter != null) {
            ...
            List<String> stringFields = Arrays.asList("warrantyTypeName");
