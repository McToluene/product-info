...
@AllArgsConstructor
@NoArgsConstructor
public class ProductCategoryFilter extends BaseFilter {
    ...
    public <T, D> void filter(QueryBuilder<T, D> queryBuilder) {
        ...
        if (!StringUtils.isEmpty(productCategoryName)) {
            queryBuilder.in("productCategoryName", Arrays.asList(productCategoryName.split(",")));
        }
    ...
    private <T, D> void filterByString(QueryBuilder<T, D> queryBuilder) {
        if (filter != null) {
            ...
            List<String> stringFields = Arrays.asList("productCategoryName");
