...
import com.mctoluene.productinformationmanagement.configuration.context.SpringContext;
...
public class QueryBuilder<T, D> {
    ...
    private QueryBuilder(EntityManager entityManager, Class<T> entityClass, Class<D> responseClass,
            boolean responseClassAsSource) {
    ...
    public static <T, D> QueryBuilder<T, D> build(Class<T> entityClass, Class<D> responseClass,
            boolean responseClassAsSource) {
        return new QueryBuilder<>(SpringContext.getBean(EntityManager.class), entityClass, responseClass,
                responseClassAsSource);
    }
    ...
    public QueryBuilder<T, D> orderBy(Order... orders) {
        criteriaQuery
                .orderBy(Arrays.stream(orders).map(order -> (order.isAsc) ? criteriaBuilder.asc(root.get(order.column))
                        : criteriaBuilder.desc(root.get(order.column))).toList());
        return this;
    }

    public QueryBuilder<T, D> orderBy(List<Order> orders) {
        criteriaQuery.orderBy(orders.stream().map(order -> (order.isAsc) ? criteriaBuilder.asc(root.get(order.column))
                : criteriaBuilder.desc(root.get(order.column))).toList());
        return this;
    }

    private void orderBy(Pageable pageable) {
        final List<javax.persistence.criteria.Order> orders = pageable.getSort().stream().map(sortOrder -> {
    ...
    private Join<Object, Object> join(Root<T> root, String model, JoinType joinType) {
    ...
    public QueryBuilder<T, D> like(String column, String value) {
        return addPredicate(criteriaBuilder.like(criteriaBuilder.lower(root.get(column)), value.toLowerCase()), false)
                .addPredicate(criteriaBuilder.like(criteriaBuilder.lower(countRoot.get(column)), value.toLowerCase()),
                        true);
    }
    ...
    public <Y extends Comparable<? super Y>> QueryBuilder<T, D> greaterThanOrEqualTo(String column, String key,
            Y value) {
    ...
    public QueryBuilder<T, D> groupBy(String... columns) {
        final Expression<?>[] expressions = Arrays.stream(columns).map(root::get).toList()
