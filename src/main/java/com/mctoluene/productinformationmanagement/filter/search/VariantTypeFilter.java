...
@AllArgsConstructor
@NoArgsConstructor
public class VariantTypeFilter extends BaseFilter {
    ...
    public <T, D> void filter(QueryBuilder<T, D> queryBuilder) {
        ...
        if (!StringUtils.isEmpty(variantTypeName)) {
            queryBuilder.in("variantTypeName", Arrays.asList(variantTypeName.split(",")));
        }
        ...
        List<String> validStatusList = Arrays.asList("ACTIVE", "INACTIVE");
    ...
    private <T, D> void filterByString(QueryBuilder<T, D> queryBuilder) {
        if (filter != null) {
            ...
            List<String> stringFields = Arrays.asList("variantTypeName");
